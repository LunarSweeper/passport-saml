...
SAML.prototype.getAuthorizeUrl = function (req, callback, finalQueryParams) {
  var self = this;
  self.generateAuthorizeRequest(req, self.options.passive, function(err, request){
    if (err)
      return callback(err);
    var operation = 'authorize';
    self.requestToUrl(request, null, operation, self.getAdditionalParams(req, operation), callback, finalQueryParams);
  });
};
