...
SAML.prototype.requestToUrl = function (request, response, operation, additionalParameters, callback, locale) {
  var self = this;
  ...
  function requestToUrlHelper(err, buffer) {
    ...
    // Add locale as a last query param because suomi.fi service
    // cannot handle it otherwise.
    target.query.locale = locale;
...
SAML.prototype.getAuthorizeUrl = function (req, callback, locale) {
  var self = this;
  self.generateAuthorizeRequest(req, self.options.passive, function(err, request){
    if (err)
      return callback(err);
    var operation = 'authorize';
    self.requestToUrl(request, null, operation, self.getAdditionalParams(req, operation), callback, locale);
  });
};
